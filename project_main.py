...
training_configuration.add_argument('--balance_factor', type=float, default=1.5,help='Specify an factor to postion head prediction loss, if set to 0, remove the PE head')
...
training_configuration.add_argument('--train_split', type=float, default=1,
                                    help='Specify amount of trainig data to be trained')
...
training_configuration.add_argument('--ssl_training', type=int, default=0, help='Specify run ssl training')
training_configuration.add_argument('--sup_ssl_withperm', type=int, default=0,help='Specify supervised training, data with permutation, backbone initilized using ssl model')
...
training_configuration.add_argument('--sup_withoutperm', type=int, default=0,help='Specify supervised training, data without permutation, backbone initilized ImageNet')
training_configuration.add_argument('--sup_withperm', type=int, default=0,help='Specify supervised training, data with permutation, backbone initilized ImageNet')
...
training_configuration.add_argument('--ranking_loss', type=str, default='CosineSimilarity',help='Specify position embedding loss name')
...
training_configuration.add_argument('--weight_decay', type=float, default=4e-4,help='Specify if to use weight decay regularization in optimizer')
...
training_configuration.add_argument('--weight_decay_ssl', type=float, default=4e-4,help='Specify if to use weight decay regularization in optimizer')
...
# require to download data
    ...
    train_df, train_data = parse_train_data(task_name=task_name, folder_path=train_folder_path, train='trainval',
                                            current_folder=current_folder)
    test_df, test_data = parse_train_data(task_name=task_name, folder_path=test_folder_path, train='test',
                                          current_folder=current_folder)
...
# generate histogram to illustrate if data is balance or not 
...
if training_configuration.ssl_training:
    ...
    # set optimizer 
    optimizer, scheduler, scheduler_worm_up = set_optimizer(model, training_configuration, train_loader,
    ...
    # set position embedding loss, and permutation index prediction loss
...
if training_configuration.sup_ssl_withperm:
    ...
    # set optimizer
    optimizer, scheduler, scheduler_worm_up = set_optimizer(ssl_model, training_configuration, train_loader,
                                                            amount_of_class=amount_of_class, alpha=alpha)
...
if training_configuration.sup_ssl_withoutperm:
    ...
    # run model to evaluate preformance 
    summary_modelresult_df = get_model_results(ssl_model, student, training_configuration, model_path, criterion,
                                               ranking_criterion, accuracy_metric, perm_creterion,
                                               train_loader, val_loader, test_loader, device,
                                               train_val_test_summary)
...
if training_configuration.sup_withoutperm:
    ...
    gc.collect()
    if training_configuration.train_model:
        train_results_df = main(model, student, optimizer, criterion, ranking_criterion, accuracy_metric,
                                perm_creterion,
                                train_loader, val_loader, num_epochs=training_configuration.epochs_count, device=device,
                                tb_writer=tb_writer, max_opt=training_configuration.max_opt,
                                model_path=model_path, scheduler=scheduler,
