...
def sellect_scheduler(optimizer, training_configuration, data_loader, scheduler_name = 'LambdaLR'):
    ...
    scheduler_bank = ['LambdaLR', 'OneCycleLR' ]
...
def step(model, data, labels, classification_criterion, accuracy_metric, optimizer=None):
    ...
    classification_loss = classification_criterion(m(classification_pred.cpu()), labels.argmax(1))
...
def eval_model(model, classification_criterion, accuracy_metric, data_loader, device):
    total_accuracy = 0.
    total_f1_score = 0.
    ...
    for idx, (data, target, perm_order, target_name) in enumerate(data_loader):
        ...
        if idx>1 and debug:
            break
        classification_loss, accuracy, f1_score = step(model, data.to(device), target, classification_criterion, accuracy_metric)
...
def train(model, optimizer, classification_criterion, accuracy_metric, data_loader, device, scheduler= None):
    total_accuracy = 0.
    total_f1_score = 0
    total_classification_loss = 0
    debug = False
    ...
    with tqdm(data_loader) as pbar:
        ...
        for idx, (data, target, perm_order , target_name)  in enumerate(pbar):
            ...
            if idx >1 and debug:
                break
            optimizer.zero_grad()
            classification_loss, accuracy, f1_score  = step(model, data.to(device), target, classification_criterion, accuracy_metric, optimizer)
            total_accuracy += accuracy*batch_size
            total_f1_score += f1_score*batch_size
            total_classification_loss += classification_loss*batch_size
...
def main(model, optimizer, classification_criterion, accuracy_metric, train_loader, val_loader, num_epochs, device, tb_writer = None, scheduler = None, model_path = ''):
    accuracy_train_list = []
    ...
    best_model_validation_f_score  = 0
    max_patience = 15
    for epoch in range(num_epochs):
        train_accuracy, train_f1_score, train_classification_loss = train(model, optimizer, classification_criterion, accuracy_metric, train_loader, device, scheduler=scheduler )
        val_accuracy, val_f1_score, val_classification_loss = eval_model(model, classification_criterion, accuracy_metric, val_loader, device)
        ...
        if val_f1_score >= best_model_validation_f_score:
            best_model_wts = copy.deepcopy(model.state_dict())
            if model_path!= '':
                torch.save(best_model_wts, model_path)

            best_model_validation_f_score = val_f1_score
            patience = 0
        patience += 1
